...
gui, add, button, x-99 y-99 vAddLine, Add Line	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Add Line" or "Add Column" buttons by moving them first  to an invisible gui area, avoiding so graphic bugs\glitches)
					;after the first 6 edit controls are added, the button will be moved to a visible and correct position

gui, add, button, x-99 y-99 vAddColumn, Add Column	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Add Line" or "Add Column" buttons by moving them first  to an invisible gui area, avoiding so graphic bugs\glitches)
					;after the first 6 edit controls are added, the button will be moved to a visible and correct position
...
GuiControlGet, Temp, %GuiCount%: Pos, C%Column%L%Line% 	;get x, y , width and height position from the last added edit control "C%Column%L%Line%" and store values in "TempY" "TempX" "TempW" "TempH" variables
						;%GuiCount% the gui id that the "Add Column" or "Add Line" Button belongs 

GuiControl, %GuiCount%: Move, AddLine, % "X" TempX "Y" TempY + TempH + 5	;%GuiCount% the gui id that the "Add Column" or "Add Line" Button belongs
GuiControl, %GuiCount%: Move, AddColumn, % "X" TempX + TempW + 5 "y" TempY

gui, show, w500 h300, Gui %GuiCount% (Main Windows)
...
gui, Test:default	;_______Gui Test__________ 
...
gui, add, button, xm w200, Add Vertical
gui, add, button, w200, Add Horizontal
x := A_ScreenWidth/2 - 40 
...
TestButtonAddVertical:		;______"Gui Test" Test Button Add Vertical______

testn++
gui, add, edit , xs  y+5 w50 r1 vTestEdit%testn%, Edit%testn%
testn++
...
UpdateScrollBars(A_Gui, Gui%A_Gui%Width, Gui%A_Gui%Height, 2, 20, 20, 20, 20)

testcount++
guicontrol,  , testeditcount, % testcount
return

TestButtonAddHorizontal:	;______"Gui Test" Test Button Add Horizontal_______

testn++
gui, add, edit , ym  w50 r1 vTestEdit%testn%, Edit%testn%
testn++
...
guicontrol, %a_gui%: Move, AddLine, y-100	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Add Line" or "Add Column" buttons by moving them first  to an invisible gui area, avoiding so graphic bugs\glitches)
...
GuiControlGet, Temp, %a_gui%: Pos, C%Column%L%Line%	;correct Y position for the new added controls to prevent gui windows blank\empty spaces  if "scrollbars != 0" !
YTemp := TempY + TempH + 5			 	;get x, y , width and height position from the last added edit control "C%Column%L%Line%" and store values in "TempY" "TempX" "TempW" "TempH" variables
						;%a_gui% the gui id that the "Add Column" or "Add Line" Button belongs 


GuiControlGet, Temp, %a_gui%: Pos, C1L1	;for correct X position necessary to prevent blank\empty spaces  if "scrollbars != 0" !
XTemp := TempX
...
UpdateScrollBars(A_Gui, Gui%A_Gui%Width, Gui%A_Gui%Height, 2, 20, 20, 20, 20)	;must use "Gui%A_Gui%Width" and "Gui%A_Gui%Height" instead just "A_GuiWidth" and "A_GuiHeight" (see more information at "GuiSize:" label in this script file)
...
guicontrol, %a_gui%: Move, AddColumn, y-100	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Add Line" or "Add Column" buttons by moving them first  to an invisible gui area, avoiding so graphic bugs\glitches)

	;gosub, ScrollUpLeft		;to prevent gui windows blank\empty spaces  if "scrollbars != 0" !
				;not necessary anymore, "XTemp" and "YTemp" below in use instead

GuiControlGet, Temp, %a_gui%: Pos, C%Column%L%Line%	;correct X position for the new added controls to prevent gui windows blank\empty spaces  if "scrollbars != 0" !
...
GuiControlGet, Temp, %a_gui%: Pos, C1L1	;for correct Y position necessary to prevent blank\empty spaces  if "scrollbars != 0" !
YTemp := TempY
...
GuiControlGet, Temp, %a_gui%: Pos, C%Column%L%Line% 	;get x, y , width and height position from the last added edit control "C%Column%L%Line%" and store values in "TempY" "TempX" "TempW" "TempH" variables
						;%a_gui% the gui id that the "Add Column" or "Add Line" Button belongs

GuiControl, %a_gui%: Move, AddLine, % "X" TempX "Y" TempY + TempH + 5		;%a_gui% the gui id that the "Add Column" or "Add Line" Button belongs
GuiControl, %a_gui%: Move, AddColumn, % "X" TempX + TempW + 5 "y" TempY
...
f11::	;_______________F12_____________

return

f12::	;_______________F11____________

Msgbox, MostLeftControl = %MostLeftControl% `nMostTopControl = %MostTopControl% `nMostRightControl = %MostRightControl% `nMostBottomControl = %MostBottomControl%
...
%A_DefaultGui%MostLeftControl := %A_DefaultGui%MostLeftControl	;tunrs The "MostLeft\Top\Right\BottomControls" variables into Dynamic variables (preserving their original values) 
%A_DefaultGui%MostTopControl := %A_DefaultGui%MostTopControl	;so, "MostLeft\Top\Right\BottomControls" values from "Main Script" are visible to "UpdateScrollBars()" function and vice-versa! 
%A_DefaultGui%MostRightControl := %A_DefaultGui%MostRightControl
%A_DefaultGui%MostBottomControl := %A_DefaultGui%MostBottomControl

if %A_DefaultGui%GuiScrollableAtExecution != 0
{
UpdateScrollBars(A_Gui, A_GuiWidth, A_GuiHeight, 1, 20, 20, 20, 20)	;"UpdateOption = 1" (Slow), The Most Left\Top\Right\Bottom controls will be detected\defined at script execution 
							...
							;in this case "AddColumn" Button is always the "MostRightControl" (at script execution and after adding new controls)
							;in this case "AddLine" Button is always the "MostBottomControl" (at script execution and after adding new controls)
%A_DefaultGui%GuiScrollableAtExecution = 0				
}
else
