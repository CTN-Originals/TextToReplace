...
UpdateScrollBars(A_Gui, A_GuiWidth, A_GuiHeight)	;update new added lines (or any other added controls) inside gui windows
...
SendMessage, 0x115, 0, 0, ,		;moves vertical scroll to windows top (to prevent "blank" areas in gui windows)
...
SendMessage, 0x114, 6, 0, ,		;moves horizontal scroll to windows left edge (to prevent "blank" areas in gui windows)
...
SendMessage, 0x115, 1, 1, , 		;moves vertical scroll to windows bottom 
				;"1" means move down ("3" moves down higher)
				;"0" means move up ("2" moves up higher)
				;"6" moves top
				;"7" moves to bottom
...
SendMessage, 0x114, 7, 0, ,	;moves horizontal scroll to windows left edge (to prevent "blank" areas in gui windows)
				;"1" means move right ("3" moves right higher)
				;"0" means move left ("2" moves left higher)
				;"6" moves left edge
				;"7" moves to right edge
...
WheelUp::
OnScroll(InStr(A_ThisHotkey,"Down") ? 10 : 1, 1, GetKeyState("Shift") ? 0x114 : 0x115, WinExist())
return
WheelDown::
; +WheelUp::
; +WheelDown::
