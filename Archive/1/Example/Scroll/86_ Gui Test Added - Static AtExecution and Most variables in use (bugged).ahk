...
gui, 2:default	;_______gui 2__________ 
...
Global MostLeftControl
Global MostTopControl
Global MostRightControl
Global MostBottomControl
...
{    			;for "UpdateOption = 1", The more controls added, More slow will be "UpdateScrollBars()" function (because of  "Loop, Parse, ..." in use)
	...
	{
	Left := cX
	MostLeftControl = %A_LoopField%
	}
        if (cY < Top)
	{
	Top := cY
	MostTopControl = %A_LoopField%
	}
        if (cX + cW > Right)
	{
	Right := cX + cW
	MostRightControl = %A_LoopField%
	}
        if (cY + cH > Bottom)
	{
	Bottom := cY + cH
	MostBottomControl = %A_LoopField%
	}
    }
}
...
guicontrolget, %MostLeftControl%, pos
Left := %MostLeftControl%X

guicontrolget, %MostTopControl%, pos
Top := %MostTopControl%Y

guicontrolget, %MostRightControl%, pos
Right := %MostRightControl%X + %MostRightControl%W

guicontrolget, %MostBottomControl%, pos
Bottom := %MostBottomControl%Y + %MostBottomControl%H
}
