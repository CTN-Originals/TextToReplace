	...
	;4 Global Dynamic variables added, The "MostLeft\Top\Right\BottomControls" Variables (to be used with "UpdateScrollBars()" function, parameter "UpdateOption = 2")
	;the 4 Global variables above are used too while loading along with "GuiScrollableAtExecution" variable
	...
	;"F11" and "F12" Hotkeys Threads can add more gui controls than "Gui Add Buttons" threads themselves! (A Limiation that even the current Lazy\Poor Developer (2 Agst 2016) of this program language doesn't know the cause!)
...
gui, add, Button, x-99 y-99 vAdd gAdd Default , Add (F12)	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Add (F12)" button by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
						;after the first 6 edit controls are added, the button will be moved to a visible and correct position

gui, add, Edit, x-99 y-99 w50 center vAddLine, 1	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "AddLine" edit control by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
					;after the first 6 edit controls are added, the button will be moved to a visible and correct position

guicontrolget, Temp, pos, Add

gui, add, Edit, x-99 y-99 w%TempW% center vAddColumn, 1	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "AddColumn" edit control by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
						;after the first 6 edit controls are added, the button will be moved to a visible and correct position

gui, add, button, w50  x-99 y-99 vMainSave, Save	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Save" button by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
					;after the first 6 edit controls are added, the button will be moved to a visible and correct position
...
gui, add, button, h17 gMoreInfo, More Info

gui, add, edit, xm w50 vC1L1 gRealTimeOutput, C1L1`n`n
...
gui, add, text, xm, Limit = Up to 11,000 Controls
gui, add, button, x+25 h17 gMoreInfo, More Info
gui, add, text, xm y+-0, Save Location = Script Directory
...
gui, add, edit, x+5 w50 vTestVertAdd gTestVertAdd, 1
gui, add, button, xm w130 vTestAddHorizontal gTestAddHorizontal, Add Horizontal (F12)
gui, add, edit, x+5 w50 vTestHorAdd gTestHorAdd, 1
...
TestAddVertical:		;______"Gui Test" Test Button Add Vertical______
	...
	{
	%A_DefaultGui%MostBottomControlAtExecution := %A_DefaultGui%MostBottomControl	;necessary for "Test Gui Save"
									;in this case, the "Most Bottom Control At Execution" should be saved in ".ini" file
									;in this case, "Edit5" control is the "MostBottomControlAtExecution"
	TestAddVertical = 0
	}
...
UpdateScrollBars(A_DefaultGui, Gui%A_DefaultGui%Width, Gui%A_DefaultGui%Height, 2, 20, 20, 20, 20)	;"A_DefaultGui" in use instead "A_Gui" in order to allow "F12" and "F11" Hotkeys threads to target the correct Gui Windows!
...
TestAddHorizontal:	;______"Gui Test" Test Button Add Horizontal_______
	...
	{
	%A_DefaultGui%MostRightControlAtExecution := %A_DefaultGui%MostRightControl	;necessary for "Test Gui Save"
									;in this case, the "Most Right Control At Execution" should be saved in ".ini" file
									;in this case, "More Info" Button is the "MostRightControlAtExecution"
	TestAddHorizontal = 0
	}
...
TestVertAdd:	;__________Test Gui (Default Add Vertical Button)____________

guicontrol, +Default, TestAddVertical

return

TestHorAdd:	;__________Test Gui (Default Add Horizontal Button)____________

guicontrol, +Default, TestAddHorizontal
...
guicontrol, Move, Add, y-100		;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Add" button by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
guicontrol, Move, AddLine, y-100		;negative x and y, so the edit control won't be visible (prevents the last  added "Edit" control to overlap "AddLine" edit control by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
guicontrol, Move, AddColumn, y-100	;negative x and y, so the edit control won't be visible (prevents the last  added "Edit" control to overlap "AddColumn" edit control by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
guicontrol, Move, MainSave, y-100	;negative x and y, so the button won't be visible (prevents the last  added "Edit" control to overlap "Save" button by moving it first  to an invisible gui area, avoiding so graphic bugs\glitches)
...
loop, %AddColumn%
{
	...
	{	
	if a_index = 1
	gui, add, edit, x%XTemp% y%YTemp% w50 r1 vC%Column%L%a_index% gRealTimeOutput, C%Column%L%a_index%
	else
	gui, add, edit, y+5 w50 r1 vC%Column%L%a_index% gRealTimeOutput, C%Column%L%a_index%	;"Y+5", "5" is the "y" spacement between new added controls (necessasy to prevents spacement problems)  
...
gosub, ButtonsReposition
	...
	;%A_DefaultGui%MostLeftControl = Specify here the new control (remove initial ";")	;in this case, after adding new controls, The Most Left\Top\Right\Bottom controls are the same controls detected while using "UpdateOption = 1" at script execution! 
	;%A_DefaultGui%MostTopControl = Specify here the new control (remove initial ";")	; - if The Most Left\Top\Right\Bottom are about to change while adding\moving controls, so they must be specified!
	;%A_DefaultGui%MostRightControl = Specify here the new control (remove initial ";")	;in this case, "Text 1" control ("Static 1") is always the "MostLeft" and the "MostTop" Control (at script execution and after adding new controls)
	;%A_DefaultGui%MostBottomControl = Specify here the new control (remove initial ";")	;in this case "Add (F12)" Button is always the "MostRightControl" and "MostBottomControl" (at script execution and after adding new controls)	

UpdateScrollBars(A_DefaultGui, Gui%A_DefaultGui%Width, Gui%A_DefaultGui%Height, 2, 20, 20, 20, 20)	;"A_DefaultGui" in use instead "A_Gui" in order to allow "F12" and "F11" Hotkeys threads to target the correct Gui Windows!
										;must use "Gui%A_Gui%Width" and "Gui%A_Gui%Height" instead just "A_GuiWidth" and "A_GuiHeight" (see more information at "GuiSize:" label in this script file)
...
GuiControl, Move, MainSave, % "X" TempX - TempW - 4 "y" TempY + TempH +5	;"-4" instead "-5", otherwise, "Save" button would be defined as the "MostLeftControl", but it should not (prevents incorrect scroll area calculations)!
...
MoreInfo:	;________________More Info_______________________
...
gui, add, text, , 
(
11000 Controls Limit:

    -  "Hotkeys threads" can reach the Limit but "Gui Threads" can't!

    - "Gui Threads" are buggy while adding multiple new controls, while "Hotkeys threads" are not!

"F11" and "F12" Hotkeys Threads can add more gui controls than "Gui Add Buttons" threads themselves!
(A Limiation that even the current Lazy\Poor Developer (2 Agst 2016) of this program language doesn't know the cause!)
)
...
gui, Wait: +owner%A_DefaultGui%	;Make "Wait" gui window owned by, in this case, "1" Main or "Test" Gui windows.
...
MouseGetPos, , , Hwnd, ControlId 	;"ControlId" is the name of the variable in which to store the name (ClassNN) of the control under the mouse cursor. If the control cannot be determined, this variable will be made blank.
				;"Hwnd" is the name of the variable in which to store the unique ID number of the window under the mouse cursor. If the window cannot be determined, this variable will be made blank.
				;"ControlId, x" where x can be 1,2 or 3 - "2 retrieves control Hwnd id number", "blank retrieves ClassNN name" 

tooltip, %bar% - %a_guicontrol% - %a_control% \ %wParam% - %lParam% - %msg% - %hwnd% \ ControlId %ControlId%

    static SIF_ALL=0x17, SCROLL_STEP=10

if  (msg = 0x115) or (msg = 0x20A)
bar = 1			;SB_VERT=1	
else
bar = 0			;SB_HORZ=0

if (wParam = 7864320) or (wParam = 7864324)
Wparam = 0
...
return
	...
	;"OnMessage(MouseWheel)" sends the Hwnd id number of the focused control and not the Window Hwnd id number!
	;this is why "MouseGetPos" above should be used to retrieve Window Hwnd id number

	;Static RunAtScriptExecution := OnMessage(0x200, "OnScroll")	;mouse movement

Static RunAtScriptExecution3 := OnMessage(0x115, "OnScroll")	;"0x115", Vertical Scroll (when a gui main scroll bar is left clicked for example)
Static RunAtScriptExecution4 := OnMessage(0x114, "OnScroll")	;"0x114", Horizontal Scroll (when a gui main scroll bar is left clicked for example)
Static RunAtScriptExecution8 := OnMessage(0x20A, "OnScroll")	;MOUSEWHEEL (Vertical) - Physical Mouse Wheel
Static RunAtScriptExecution9 := OnMessage(0x20E, "OnScroll")	;MOUSEHWHEEL (Horizontal) - Physical Mouse Wheel
}

f1::
OnMessage(0x200, "OnScroll")	;mouse movement
return