	...
	; - "UpdateScrollBars(A_Gui, Gui%A_Gui%Width, Gui%A_Gui%Height)" instead "UpdateScrollBars(A_Gui, A_GuiWidth, A_GuiHeight)"
		...
		;necessary because "A_GuiWidth" and "A_GuiHeight" only return the correct gui windows size if "GuiSize" lable is launched by resizing gui windows, otherwise, incorrect 0 and 32768 values will be returned!
...
gui, add, edit, x+5  w50  vC%a_index%L%Line% gRealTimeOutput, C%a_index%L%Line%
	...
	;GuiScrollableMostRightControl =	Specify here the new control (remove initial ";")		;in this case, after adding new controls, "GuiScrollableMostRightControl" and "GuiScrollableMostBottomControl" is\are the same control\s detected while using "UpdateOption = 1" at script execution! 
	;GuiScrollableMostBottomControl	= Specify here the new control (remove initial ";")	;(if "GuiScrollableMostRightControl" or "GuiScrollableMostBottomControl" is\are about to change while adding new controls, the new "GuiScrollableMostRightControl" or "GuiScrollableMostBottomControl" must be specified)
									;in this case "AddColumn" Button is always the "GuiScrollableMostRightControl" (at script execution and after adding new controls)
									;in this case "AddLine" Button is always the "GuiScrollableMostBottomControl" (at script execution and after adding new controls)

UpdateScrollBars(A_Gui, Gui%A_Gui%Width, Gui%A_Gui%Height, 2, 20, 20)	;"UpdateOption = 2" to be used while adding new controls (Recalculates Scroll Width\Height Area while adding new controls)
							;"20" "RightMargin" and "BottomMargin" = 20 (must be the same value for any "UpdateScrollBars()" in use)
...
Gui%A_Gui%Width = %A_GuiWidth%	;necessary because "A_GuiWidth" and "A_GuiHeight" only return the correct gui windows size if "GuiSize" lable is launched by resizing gui windows, otherwise, incorrect 0 and 32768 values will be returned!
...
Global GuiScrollableMostRightControl
Global GuiScrollableMostBottomControl

if GuiScrollableAtExecution != 0
{
UpdateScrollBars(A_Gui, A_GuiWidth, A_GuiHeight, , 20, 20)	;"UpdateOption = blank", so, the "MostRightControl" and "MostBottomControl" will be detected\defined at script execution
						;"20" "RightMargin" and "BottomMargin" = 20 (must be the same value for any "UpdateScrollBars()" in use)
						;in this case "AddColumn" Button is always the "GuiScrollableMostRightControl" (at script execution and after adding new controls)
						;in this case "AddLine" Button is always the "GuiScrollableMostBottomControl" (at script execution and after adding new controls)
...
UpdateScrollBars(A_Gui, A_GuiWidth, A_GuiHeight, 1, 20, 20)	;"UpdateOption = 1" Update Scroll bars if gui windows is resized 
						;"20" "RightMargin" and "BottomMargin" = 20 (must be the same value for any "UpdateScrollBars()" in use)
...
if UpdateOption =	;if "UpdateOption" = blank (detects\define the "Most Right" and the "Most Bottom" gui control at script execution)
{   
...
if UpdateOption = 1			;Update Scroll bars if gui windows is resized 
{
   ;Calculate Scrolling Area
...
Left := NewRight - RightGuiScrollable 
Top := NewBottom - BottomGuiScrollable
Right := RightGuiScrollable + Left		;("+" because "Left" value is always <= 0, negative or 0, so, "Left" will be subtracted from "RightGuiScrollable")
Bottom := BottomGuiScrollable + Top	;("+" because "Top" value is always <= 0, negative or 0, so, "Top" will be subtracted from "BottomGuiScrollable")

	;msgbox, %Left% - %Top% - %Right% - %Bottom%
}

if UpdateOption = 2		;Recalculates Scroll Width\Height Area while adding new controls
{
...
Left := 0
Top := 0
Right := NewRight + GuiScrollableHorizontalPos
Bottom := NewBottom + GuiScrollableVerticalPos
